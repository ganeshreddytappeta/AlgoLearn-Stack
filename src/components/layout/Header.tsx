...
export const Header: React.FC<HeaderProps> = ({
  ...
  const activeMeta = tabTitles[currentTab] || tabTitles.home;
  ...
  return (
    <header className="shrink-0 sticky top-0 z-30 min-h-[64px] sm:min-h-[72px] bg-white dark:bg-slate-900 border-b border-slate-200/90 dark:border-slate-800 px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4 transition-colors shadow-2xs">
      <div className="flex items-center gap-3 sm:gap-4">
        {/* Toggle Navigation Hamburger Button */}
        <button
          id="nav-toggle-btn"
          onClick={() => {
            soundEffects.playClick();
          ...
          title={isSidebarOpen ? 'Close Navigation Menu' : 'Open Navigation Menu (Ctrl+B)'}
        ...
        <div>
          <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight leading-tight">
            {activeMeta.title}
          </h2>
